...
def show_confMat(confusion_mat, classes, set_name, out_dir):
    ...
    # 获取颜色
    cmap = plt.cm.get_cmap('coolwarm')  #bwr 更多颜色: http://matplotlib.org/examples/color/colormaps_reference.html
    plt.figure(figsize = (20,20))
    plt.imshow(confusion_mat_N, cmap=cmap)
    plt.colorbar(shrink = 0.8)
    
#    sns.heatmap(confusion_mat_N, vmin = 0, vmax = 200, center = 0)
#    plt.show()
#    
    # 设置文字
    xlocations = np.array(range(len(classes)))
    plt.xticks(xlocations, list(classes), rotation=90)
    plt.yticks(xlocations, list(classes))
    plt.xlabel('Predict label')
    plt.ylabel('True label')
    plt.title('MLFCNeXt50')#'Confusion_Matrix_' + set_name)#

    # 打印数字
    for i in range(confusion_mat_N.shape[0]):
        for j in range(confusion_mat_N.shape[1]):
            plt.text(x=j, y=i, s=int(confusion_mat[i, j]), va='center', ha='center', color='white', fontsize=10)
    # 保存
